...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'cityPages.orlando' });
...
export default async function OrlandoPage({ params }: Props) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'cityPages.orlando' });
  const tCityPage = await getTranslations({ locale, namespace: 'cityPage' });

  // Fetch city data from Sanity
  const cityFromSanity = await getCityBySlug('orlando');
    ...
    hasOfferCatalog: {
      ...
      itemListElement: services.map(service => ({
        ...
        itemOffered: {
          "@type": "Service",
          name: service,
          description: `Professional ${service.toLowerCase()} services in ${t('name')}`
        }
      }))
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
              {services.map((service, index) => (
                <div key={index} className="bg-white p-6 rounded-lg shadow-sm">
                  ...
                  <p className="text-gray-600">{tCityPage('serviceDescription', { service: service.toLowerCase(), city: t('name') })}</p>
                </div>
              ))}
            </div>
          </div>
